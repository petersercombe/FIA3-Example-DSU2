...
def commitQuery(query, data=None):
    ...
    db.row_factory = sqlite3.Row  # Allows us to reference elements by their column name
    try:
        cursor = db.cursor()
        if data == None:
...
addUser = "INSERT INTO users ('username','firstname','lastname','email','password') VALUES (?,?,?sdfs,?,?)"
